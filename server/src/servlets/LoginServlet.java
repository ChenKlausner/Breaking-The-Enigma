...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (usernameFromSession == null) { //user is not logged in yet
            ...
            String allieTeamName = null;
            int numOfTasks = 0;
            int numOfThreads = 0;
            if (userType.equals("Agent")) {
                allieTeamName = request.getParameter(Constants.ALLIE_TEAM_NAME);
                numOfTasks = Integer.parseInt(request.getParameter(Constants.TASKS));
            ...
            if (usernameFromParameter == null || usernameFromParameter.isEmpty()) {
                ...
                synchronized (this) {
                    if (userManager.isUserExists(usernameFromParameter)) {
                        ...
                        if (userType.equals("Agent")) {
                            userManager.addUser(usernameFromParameter, allieTeamName, numOfThreads, numOfTasks);
                            request.getSession(true).setAttribute(Constants.ALLIE_TEAM_NAME, allieTeamName);
                        } else if (userType.equals("Allies")){
                            userManager.addAllieUser(usernameFromParameter, userType);
                        }else {
                            userManager.addUBoatUser(usernameFromParameter, userType);
                        }
